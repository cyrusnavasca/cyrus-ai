...
the page's job, not something to ask the user to format.
  ...
  modal deploy deploy/serve.py     # stable URL, survives this shell
  modal serve  deploy/serve.py     # live-reloads while editing
...
def runs() -> list:
    ...
    The gate is `training_config.json`'s format field rather than the directory
    name. Earlier runs on this volume were trained to restyle a sentence rather
    than to answer one; served here they parrot whatever you typed back at you
    in the right voice, which reads as a broken chat model. They also sorted
    above the real runs by name, so leaving them in made one of them the default
    the page opened on.
    """
    import json
