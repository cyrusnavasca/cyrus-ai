"""Build (context -> my reply) training pairs straight from real threads.

The model is taught "here is the conversation, say what I would say next", so it
has to produce the content rather than restyle content it was handed.
...
from ..common.rank import stable_rank
    ...
    for thread_id, messages in by_thread.items():
        ...
        for i, turn in enumerate(turns):
            ...
            pairs.append(
                {
                    "id": hashlib.sha1(
                        f"{thread_id}:{turn['ts']}:{turn['text']}".encode()
                    ).hexdigest()[:16],
                    "context": context,
                    "output": turn["text"],
                    "meta": {
