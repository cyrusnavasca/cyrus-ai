...
Requires a CUDA GPU - Unsloth does not run on Apple Silicon. Run it on Colab,
Runpod, Lambda or any rented box, or let `deploy/train.py` rent one on Modal:

  pip install -e ".[train]"
  style-ft train --train data/processed/chat_train.jsonl \
      --eval data/processed/chat_test.jsonl --output-dir outputs/chat-v3

Smoke test the loop first - a handful of steps on a tiny base model proves the
whole path without waiting on a real run:

  style-ft train --train data/dummy/chat_train.jsonl \
...
from ..common.jsonlio import log, read_jsonl
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--my-name", default="Me",
                    help="persona name; must match the value used at inference")
    return ap
...
def main(argv: list[str] | None = None) -> int:
    ...
    trainer_kwargs = {
        ...
        "eval_dataset": eval_ds, "args": sft_config,
    }
    try:
        ...
        json.dumps(
            {
                ...
                # Read back by deploy/serve.py to decide what it can serve.
                "format": "chat",
